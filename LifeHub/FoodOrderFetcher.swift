...
class FoodOrderFetcher {
    ...
    func fetchFoodOrders(from urlString: String) async throws -> [Date: OrderData] {
        ...
        print("Fetching food orders from URL: \(url)")
        ...
        do {
            ...
            print("Error fetching data from URLSession: \(error)")

            throw FoodOrderFetcherError.networkError(error)
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            ...
            print("HTTP Error: Status code is \((response as? HTTPURLResponse)?.statusCode ?? -1)")
            ...
            throw FoodOrderFetcherError.parsingError("Could not decode data into UTF-8 string.")
                ...
                for (index, line) in lines.enumerated() {
                    ...
                    guard let date = dateFormatter.date(from: parts[dateIndex]) else {
                        ...
                        print("Warning: Invalid date format in row \(index + 2). Raw date string: '\(parts[dateIndex])'")
